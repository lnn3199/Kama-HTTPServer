...
namespace {
std::string maskApiKey(const std::string& k) {
    if (k.empty()) return "(empty)";
    if (k.size() <= 12) return "***";
    return k.substr(0, 4) + std::string("...") + k.substr(k.size() - 4);
}
}  // namespace

// 构造函数：不在此处 create("1")，否则会强制读 DASHSCOPE_API_KEY；
// 实际策略在 chat() 里按 modelType 设置（豆包=2 等），避免只配了豆包仍报阿里云 Key。
AIHelper::AIHelper() = default;
...
json AIHelper::executeCurl(const json& payload) {
    ...
    std::string modelInPayload = "?";
    if (payload.contains("model") && payload["model"].is_string())
        modelInPayload = payload["model"].get<std::string>();
    std::cout << "[AI] POST " << strategy->getApiUrl() << " model=" << modelInPayload
              << " key=" << maskApiKey(strategy->getApiKey()) << std::endl;
    ...
    std::cout << "[AI] HTTP " << http_code << " response_len=" << readBuffer.size() << std::endl;
